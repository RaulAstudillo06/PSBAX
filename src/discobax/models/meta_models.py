...
from typing import Any, List, AnyStr, Union, Type, Optional
...
class SklearnRandomForestRegressor(AbstractMetaModel, SklearnModel):
    ...
    def get_samples(self, x: np.ndarray, k: Optional[int] = 1):
        #TODO: implement batch-wise evaluation.
    ...
    def predict(self,
        ...
        """
        Args:
            return_std: If True, return the epistemic uncertainty of the output.
...
class PytorchMLPRegressorWithUncertainty(AbstractMetaModel, EmbeddingRetrievalModel):
    """The mlp regressor model from pytorch (deep models)"""
    ...
    def get_model_prediction(self,
        ...
        if return_multiple_preds:
            y_preds = self.get_samples(data, num_target_samples)
    ...
    def get_mean_var_pred(self, data):
        y_samples = self.get_samples(data, self.num_target_samples)
        y_mean = np.mean(y_samples, axis=1)
        y_var = np.var(y_samples, axis=1)
        y_preds = self.model.predict(data)
    ...
    def predict(self,
                ...
                return_multiple_preds: bool = False,
                row_names: List[Any] = [],
                num_target_samples: int = None) -> List[np.ndarray]:
        """
        Args:
            return_std: If True, return the epistemic uncertainty of the output.
        ...
        while len(available_indices) > 0:
            ...
            if return_std_and_margin:
                y_pred = self.get_model_prediction(data, return_multiple_preds=True, num_target_samples=num_target_samples)
                y_preds.append(y_pred)
            else:
    ...
    @classmethod
    def load_folder(cls: Type["TarfileSerialisationBaseModel"], save_folder_path: AnyStr) -> AbstractBaseModel:
        print(f"Loading model checkpoint from: {save_folder_path}")
