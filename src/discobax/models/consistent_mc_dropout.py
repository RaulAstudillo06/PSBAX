...
NOTICE: 
The code used here is based on the Source work from A. Kirsch at: 
https://github.com/BlackHC/batchbald_redux/blob/master/batchbald_redux/consistent_mc_dropout.py
This file was modified to match Tensor shape expectations in GeneDisco and the GeneDisco licensing header was
added. The Source work did not include a license header but was also licensed under Apache 2.0.
"""
...
class BayesianModule(Module):
    ...
    def forward(self, input_tensors: List[torch.Tensor], k: int = 1) -> List[torch.Tensor]:
